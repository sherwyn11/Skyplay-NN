    ...
    def fit(self, X, Y, epochs):
        ...
        for i in range(0, epochs):
            ...
            if self.optimizer == "Adam":
                self.parameters, self.v, self.s = adam.update_parameters(
                    self.parameters, grads, self.v, self.s, 1, self.learning_rate
                )
            elif self.optimizer == "GD":
                self.parameters = gradient_descent.update_parameters(
                    self.parameters, grads, self.learning_rate
                )
            elif self.optimizer == "GD_momentum":
                self.parameters, self.v = gd_with_momentum.update_parameters(
                    self.parameters, grads, self.v, 0.9, self.learning_rate
                )
